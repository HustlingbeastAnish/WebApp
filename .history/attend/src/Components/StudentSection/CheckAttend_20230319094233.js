...
const CheckSub = (props) => {
  ...
  const getSubjects = () => {
    axios
      ...
      .then((res) => {
        ...
        for (let i = 0; i < StudSubjectsRef.length; i++) {
          const currSubject = StudSubjectsRef[i];
          const temp = currSubject + "_" + currBranch;
          axios
            .get(
              `http://localhost:3002/api/classesddata/${StudSubjectsRef[i]}/${currBranch}`
            )
            .then((res) => {
                  ...
                  setTotalAttendancePerSubject((TotalAttendancePerSubject) => ({
                    ...
                    [i]: (
                      ((TotalClassesPerSubject[i] -
                        res.data[StudSubjects[i]].length) /
                        TotalClassesPerSubject[i]) *
                      100
                    ).toFixed(2),
                  }));
                  console.log("Attendance As");
                  console.log(TotalAttendancePerSubjectRef[i]);
  ...
  // console.log(TotalClassesPrSubject);
  useEffect(() => {
    getSubjects();
  }, []);
  return (
