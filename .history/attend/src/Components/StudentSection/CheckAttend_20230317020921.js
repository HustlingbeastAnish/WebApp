...
const CheckSub = (props) => {
  ...
  let [StudSubjects, SetStudentSubjects] = useState(["DSA", "DEV", "WEB3"]);
  ...
  const FetchAttendanceDetails = (subject) => {
    ...
    axios
      ...
      .then((res) => {
        ...
        //   ...absentcount,
        //   [idx]: (
        //     ((totalnoofclasses - res.data[subject].length) / totalnoofclasses) *
        //     100
        //   ).toFixed(2),
        // }));
      })
  ...
  const getSubjectsAgain = async () => {
    for (let i = 0; i < StudSubjects.length; i++) {
      ...
      FetchAttendanceDetails(StudSubjects[i]);
    }
    setflag2(false);
  };
  useEffect(() => {
